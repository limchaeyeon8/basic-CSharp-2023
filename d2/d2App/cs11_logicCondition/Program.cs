...
namespace cs11_logicCondition
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            if (a == 20)        // 처리하는 로직이 있으면 무조건 {} 쓰세요!! // 중괄호 중요 - 안전한 코드
            {
            ...
            {
                case int i:         // 정수라면
                    Console.WriteLine("{0}는(은) int 형식입니다.", i);
                    break;
                case float f:       // 실수라면
                    Console.WriteLine("{0}는(은) float 형식입니다.", f);
                    break;
                case string s:      // 문자열이면
                    Console.WriteLine("{0}는(은) string 형식입니다.", s);
                    break;
